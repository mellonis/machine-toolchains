//! Drift guard for `docs/pmt/cli.md`'s error-code inventory: the page
...
use mtc_post_machine::compiler::CompileErrorKind;
...
fn the_published_compile_catalog_lists_exactly_the_registry_codes() {
    ...
    assert_eq!(
        documented, registry,
        "docs/pmt/cli.md (compile errors) and CompileErrorKind::CODES \
...
#[test]
fn the_assembly_section_points_at_the_shared_catalog_instead_of_forking_it() {
    let doc = doc();
    let lines = section(&doc, "### Assembly errors");
    assert!(
        lines
            .iter()
            .any(|l| l.contains("docs/core.md (error codes)")),
        "docs/pmt/cli.md (assembly errors) should cite the shared \
         catalog at docs/core.md (error codes)"
    );
    assert!(
        table_codes(&lines).is_empty(),
        "docs/pmt/cli.md (assembly errors) should not carry its own \
         code table — the shared catalog lives on docs/core.md, where \
         the core crate's guard pins it"
    );
}
