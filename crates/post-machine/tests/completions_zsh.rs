...
fn rendered_script() -> String {
    execute(&args(&["completions", "zsh"]))
        .expect("pmt completions zsh should succeed")
        .stdout
}
...
fn zsh_completions_output_looks_like_a_compdef_script() {
    ...
    assert!(script.starts_with("#compdef pmt"), "{script}");
    assert!(script.contains("_pmt() {"), "{script}");
    assert!(script.trim_end().ends_with("_pmt \"$@\""), "{script}");
}
...
fn zsh_completions_load_under_compinit_without_errors() {
    ...
    fs::write(zfunc.join("_pmt"), rendered_script()).unwrap();
    ...
    assert!(
        stdout.contains("_pmt: function") || stdout.contains("_pmt: autoload"),
        "compinit did not register `_pmt` as a completion function: {stdout}"
    );
}
