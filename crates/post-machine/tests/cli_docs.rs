//! Drift guard for `docs/pmt/cli.md`: the page quotes every subcommand's
...
//! nothing failing. This file is that check (the twin of the tmt page's
//! guard in the sibling TM-1 crate).
//!
//! What is checked:
...
//!  - `lsp` is probed as `lsp --help` and NEVER bare — a bare `pmt lsp`
...
//!    `--help` too (`pmt tape build --help` prints usage, same as any
//!    leaf), but every action in a group renders that group's ONE shared
//!    usage text, byte-identical to the bare group's own — so probing
//!    bare is simpler and exercises the same string.
//!
...
use mtc_post_machine::completions::registry::registry;

fn doc() -> String {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../docs/pmt/cli.md");
...
fn help(argv: &[&str]) -> String {
    ...
    execute(&owned)
        .unwrap_or_else(|e| panic!("`pmt {}` should render help, got: {e}", argv.join(" ")))
        .stdout
}
...
fn every_quoted_usage_block_is_byte_identical_to_the_real_help_output() {
    ...
    for (_, argv) in quoted_blocks() {
        ...
        assert!(
            ...
            "docs/pmt/cli.md no longer quotes `pmt {}`'s help text verbatim.\n\
             The binary renders:\n{expected}\n\
...
fn every_top_level_subcommand_has_a_quoted_block_on_the_page() {
    ...
    for name in &subcommands {
        assert!(
            ...
            "`{name}` is a real subcommand but docs/pmt/cli.md quotes no usage block \
...
/// `docs/pmt/cli.md (pmt fmt)` resolves to a real heading rather than to an
...
fn every_subcommand_has_its_own_heading() {
    ...
    for (name, _) in quoted_blocks() {
        ...
        let wanted = format!("## `pmt {name}`");
        assert!(
            headings.contains(&wanted.as_str()),
            "docs/pmt/cli.md should carry the heading `{wanted}` so citations \
             naming `pmt {name}` resolve to a heading; headings are: {headings:?}"
        );
    }
}
