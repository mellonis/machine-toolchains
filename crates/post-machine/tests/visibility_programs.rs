...
//! and aliases, absolute qualified calls, binding collisions, and the
//! entry rule for namespaced `main`.
...
fn visibility_flip_changed_no_linked_bytes() {
    // Same sample and byte counts as the inline-optimization golden:
    // visibility/symbol-kind bookkeeping must never move generated bytes.
    let src = "\
goToEnd() {
1:  right;
    check(1, 2);
