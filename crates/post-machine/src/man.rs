//! The `pmt(1)` manual page, rendered as `man(7)` roff (docs/pmt/cli.md
//! (pmt man)). Library code: `cli::man` hands the string to `bin/pmt.rs`
//! to print, per the thin-renderer rule. Mirrors the TM-1 `tmt` renderer
//! (`crates/turing-machine/src/man.rs`) line for line but the names.
//!
...
pub fn render() -> String {
    ...
    out.push_str(
        ".SH SEE ALSO\n\
         .BR tmt (1),\n\
         the Turing\\-machine toolchain of the same family.\n\
         .PP\n\
...
mod tests {
    ...
    fn usage_text_covers_the_root_and_every_registered_subcommand() {
        assert!(
            usage_text(&[]).is_some_and(|u| u.starts_with("pmt — ")),
        ...
        for name in top_level_names() {
            ...
            assert!(
                text.starts_with(&format!("USAGE: pmt {name}")),
    ...
    fn opens_with_a_th_line_carrying_the_release_date_and_version() {
        ...
        assert!(
            page.starts_with(&format!(
                ".TH PMT 1 \"{RELEASE_DATE}\" \"pmt {version}\" \"User Commands\"\n"
            )),
            "{page}"
        );
    }
    ...
    fn every_subcommand_has_a_section_quoting_its_usage_verbatim() {
        let page = render();
        for name in top_level_names() {
            let heading = format!(".SS \"pmt {name}\"\n.nf\n");
            let start = page
                .find(&heading)
    ...
    fn escaping_protects_leading_dots_hyphens_and_backslashes() {
        assert_eq!(
            escape(".pmc source -> .pmo object"),
            "\\&.pmc source \\-> .pmo object"
        );
        assert_eq!(escape("  -o OUT.pmo"), "  \\-o OUT.pmo");
    ...
    fn exit_status_and_see_also_close_the_page() {
        ...
        assert!(page[see..].contains(".BR tmt (1)"), "{page}");
