//! `tmt dap`: runs the Debug Adapter Protocol server for a `.tmx` program
...
//! (docs/tmt/cli.md (thin-renderer rule)); the server loop itself writes
//! protocol frames straight to the writer it is given. The launch schema
//! and protocol surface `TmDapAdapter` answers are documented at
//! docs/dap.md.
...
pub(super) fn dap(raw: &[String]) -> Result<CliOutput, String> {
    ...
    let mut adapter = crate::dap::TmDapAdapter::new();
