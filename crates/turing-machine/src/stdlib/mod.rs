...
use crate::compiler::{
    CompileOptions, Declarations, ReadMode, Resolved, WorldKind, analyze_staged_with,
    analyze_with_mode, compile,
};
...
/// generated by `tmt interface` over [`SOURCE`] and never hand-edited
/// (docs/tmt/stdlib.md (the shipped header)): pinned byte-for-byte by
/// `stdlib_header.rs`'s `std_tmh_is_what_tmt_interface_prints` and
/// regenerated only by that file's `#[ignore]`d `regen` test.
pub const HEADER_SOURCE: &str = include_str!("std.tmh");
...
/// The embedded standard library's DECLARATIONS, as its own committed
/// header ([`HEADER_SOURCE`]) — parsed once per process, through the SAME
/// declarations-only reader a `.tmh` on `--extern` goes through, into a
/// [`Resolved`]. This is the external context [`Declarations::stdlib`]
/// clones from, in place of [`resolved`] (the analysis of [`SOURCE`]
/// itself, bodies and all): the two differ in WHAT they analyze but must
/// agree on every exported routine's declared `writes` contract, since
/// that is all the footprint inference ever reads off an external module
/// (docs/tmt/language.md (declarations)) —
/// `stdlib_header.rs`'s `the_header_and_the_source_agree_on_every_
/// declared_contract` pins exactly that agreement.
///
/// `Declarations::none()`: parsing the header must not consult
/// `Declarations::stdlib()` — this very cache — while it is being built,
