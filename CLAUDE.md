...
| `.tmc` language / TM-1 `.tma` dialect | 0.1 / 0.4 |
| PM IR / TM IR | 4 / 3 |
...
- **`crates/turing-machine` (`mtc-turing-machine`)** — everything TM-1 (arch id `0x02`): the arch module (`Tm1::new(tape_count)`, 20 opcodes — the base set plus `trap`, the framed `call.m`/`retx`, and the fused `wrmv`; batch `rd` over all heads, `mtc`/`djmp` table dispatch, per-tape `wr`/`mov` vectors with `-` keep and `<`/`>`/`.` moves; MR written only by `mtc`), the `.tma` dialect (`tm1_syntax()`, caps all on), the `.tmc` front end, the embedded stdlib twins, both lint layers and the `.tmc` formatter, `tmt.json` + the `tmt build` driver, the completions registry, both `LanguageService`s, the DAP adapter, and the `tmt` binary (fourteen subcommands, same exit codes as `pmt`).
...
- **The text-expressibility gate**: everything the compiler can put in an object is expressible in hand-written assembly, proven by dis→asm byte-identity. Three declared exceptions: `-g` debug side-tables, and exported and imported alphabets — both are compiler facts with no directive, printed as disassembly comments that reassembling drops (`docs/formats.md` (text-expressibility caveats) carries the full list). A feature that can only be emitted, never written by hand, breaks it.
