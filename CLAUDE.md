...
**All three mono stamping refusals — `MonoRawFrame`, `MonoHoleyMatchBranch` and `RecursiveExitBearingCall` — advise `--call-mech=frames`, never hybrid.** Hybrid's classifier inspects top-level sites only and delegates wholesale to mono through its `!any_frames` fast path, so advising hybrid would be circular; the recursion refusal adds a second reason — hybrid lowers that shape only when its byte rule happens to share the group. Probed, and pinned by link tests in both worlds.
