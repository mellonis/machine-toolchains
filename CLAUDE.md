...
**Thin-renderer rule: library code never prints.** Every stage returns a structured report (`CompileReport`, `LinkReport`, `OptReport`, `RunResult`); every byte of terminal output originates in `cli/` (rendered under `-v`), and errors flow as typed values. `bin/pmt.rs` is a shell around `cli::execute`. Nine subcommands split across `build.rs` (compile/asm/link), `inspect.rs` (dis/tape/ir), `run.rs` (run, incl. live `--trace`), `completions.rs` (completions), `lint.rs` (lint).
