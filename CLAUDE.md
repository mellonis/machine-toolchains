...
cargo build --workspace --lib --target wasm32-unknown-unknown   # wasm32 gate: the four libraries stay buildable for the browser (docs/core.md (async session))
...
- **`crates/turing-machine` (`mtc-turing-machine`)** — everything TM-1 (arch id `0x02`): the arch module (`Tm1::new(tape_count)`, 20 opcodes — the base set plus `trap`, the framed `call.m`/`retx`, and the fused `wrmv`; batch `rd` over all heads, `mtc`/`djmp` table dispatch, per-tape `wr`/`mov` vectors with `-` keep and `<`/`>`/`.` moves; MR written only by `mtc`), the `.tma` dialect (`tm1_syntax()`, caps all on), the `.tmc` front end, the embedded stdlib twins, both lint layers and the `.tmc` formatter, `tmt.json` + the `tmt build` driver, the completions registry, both `LanguageService`s, the DAP adapter, and the `tmt` binary (fifteen subcommands, same exit codes as `pmt`; `pmt`'s own set stays at fourteen — `interface` is TM-only).
- **`crates/wasm` (`mtc-wasm`)** — the browser binding: three wasm-bindgen classes (`Toolchain`, `Program`, `Session`) over a plain-Rust `inner/` layer (positions → UTF-16, the lint/format/build channels over all four languages — `Lang` is an architecture crossed with a kind, and `build` on `pma`/`tma` is the assembler with its line table; listing rows; the tape-block codec plus by-glyph seed mapping; a pumped `Session` over `AsyncSession` with owned `WideTape`s; one leaked `ArchRegistry` per thread — effectively one, wasm being single-threaded; `Arch` is not `Sync`, so a plain static cannot hold it (`Tm1` is width-agnostic) — for the `'static` lifetimes a class needs). **It links its own debug-compiled stdlib** (`inner/stdlib.rs`, code bytes pinned equal to the arch crates' release-preset object) and stamps `user`/`std` provenance on every link, which is what lets a stdlib address resolve to a line of `stdlibSource`. The only crate carrying `wasm-bindgen` (pinned exactly, and the CLI must match) and `js-sys`; `inner/` is held free of them by a test. Reference: `docs/wasm.md`.
...
**One parse path, source → green tree → extraction.** The hand-written
CST both crates used to build alongside the green tree is deleted;
nothing constructs or reads one anywhere. `analyze`/`analyze_staged`
run `lex_with(WithComments)` → `parse_green_from_tokens` →
`syntax::extract_program`, and so do the embedded stdlib's roster and
the build driver's source scan. Lint follows for free — `LintContext`
carries tokens and the flattened AST. `fmt` runs the same
`parse_green_from_tokens` but skips extraction, printing straight from
the raw tree and its trivia (extraction flattens the tree and drops
trivia). `parser::parse` is a thin wrapper over that identical route —
lex, green parse, extract — kept for tests and for the optimizer's,
IR's and codegen's own `#[cfg(test)]` modules. The green tree is held
to `text() == source` over the shipped corpus and over generated
programs; extraction's own fidelity is pinned by written-out literals
in `syntax::extract`'s unit tests and by the goldens, not by a second
front end to compare against.

### The `.tmc` front end (`turing-machine/src/`)

`lexer` → `parser` → `compiler` (flatten + checks) → `expand` (graft splicing and range expansion, compiler-side stamping, oracle-property-tested) → per-world state-graph `ir` → `optimizer` → `codegen` → core asm. `parser::parse_green`/`parse_green_from_tokens` seed the same parser walk with a green sink, yielding a lossless green tree held to `text() == source` over the shipped corpus and over generated programs; typed views over it (`syntax::views`) and `syntax::extract_program` build the `Program` every consumer downstream reads. **The compiler front runs that path**: `compiler::analyze` is `lex_with(WithComments)` → `parse_green_from_tokens` → `extract_program`, with each broken source's fatal pinned by written-out kind AND span, and the significant-token filter pinned equal to a comment-free lex over every `.tmc` the repo ships; `analyze_staged` (the language-service entry) now builds `program` by the same route and is pinned to agree with `analyze` field-for-field on a clean source (`tokens` compared directly, not each against a third party) and, at every broken stage, on the WHOLE fatal — kind and span, not just its code. Its `Analysis.tokens` therefore carries comments, and `Analysis.green` retains the tree itself: `LintContext` carries the tree's root and a `TextLineIndex`, never a comment-free token stream, and every quickfix span is a range query in `lint/rules/spans.rs` — the innermost node of the wanted kind containing an anchor the resolved module keeps, then that node's own range (`decl_span::<AlphabetView | ReuseView | GraftView | BindView>`) or a token pair inside it (`as_clause_span`, `marker_span`, `arrow_span`). A comment anywhere in a declaration lands inside the span, where `run_rules`' comment guard withholds the fix; no helper can be voided or truncated by one, which is why the guard is the only comment mechanism left. **`fmt` runs the green tree too** — `crate::fmt::format` is `lex_with(WithComments)` → `parse_green_from_tokens` → `fmt/print.rs`, which walks the raw tree and asks `fmt/trivia.rs` for the derived layout facts extraction drops. There is no second parse to compare against in either crate: the hand-written CSTs are deleted, and `parser::parse` is now the same lex → green parse → extract route under a convenience name. `compiler.rs`'s staged-vs-batch test uses it that way — it checks that `analyze_staged`'s tiering reaches the same extracted `Program` an unstaged `parse` does, not that two front ends agree. Language rules that are design, not implementation detail (`docs/tmt/language.md`):
...
**Declarations and headers.** `declarations.rs` holds the one declarations table every consumer reads through: `compile --extern`, `tmt build`'s derived sibling/library/stdlib set (a fixpoint, so order and chain depth do not matter), and `tmt interface` itself. Strictness is by EXTENSION, never by a second grammar — a `.tmh` is the `.tmc` grammar under `ReadMode::DeclarationsOnly` (a `machine` block and a bodied routine are errors, a graph must carry its body), a `.tmc` read for declarations is lenient. `header.rs` is the printer, two arms over one renderer (source / object), byte-pinned over the stdlib; `std.tmh` is generated output, regenerated by an `#[ignore]`d test and never hand-edited. With declarations in hand a call/bind into another unit binds tapes symbolically, an alphabet/map/graph is nameable across units, and a library graph is graftable — each side recording a CRC-32 the linker re-checks (`GraftDrift`, `AlphabetDrift`). Headers carry no `volatile` and no `preserves`; every tape publishes an effective-or-inferred `writes` set.
...
`tmt link --call-mech = mono | frames | hybrid` selects how declarative binding calls lower. The compose model is runtime `FR' = compose[FR][site]` through a composite directory in the MX v2 frames region: `call.m` operands are call-site indices, FR is a composite index, and raw hand-authored sites are constant compose columns. `mono` stamps specialized copies instead (rmap-preimage row rewriting, synthesized trap rows, one-way row expansion, digest-named dedup). The composition algebra (`core/src/linker/compose.rs`) is law-property-tested against a brute-force oracle, and the three modes are proven equivalent by the everything-matrix plus the three-mechanism `.tma` matrix (`turing-machine/tests/link_matrix.rs`) over exit vectors, open bindings and cross-object calls.

**All four copy-path refusals — `MonoRawFrame`, `MonoHoleyMatchBranch`, `RecursiveExitBearingCall` and `ExitBearingTailCall` — advise `--call-mech=frames`, never hybrid.** Hybrid's classifier inspects top-level sites only and delegates wholesale to mono through its `!any_frames` fast path, so advising hybrid would be circular; the two exit-bearing refusals add a second reason — hybrid lowers those shapes only when its byte rule happens to share the group. Probed, and pinned by link tests in both worlds, with a frames arm proving the escape.

Exit-bearing bound calls (a callee taking state parameters) lower per mechanism: frames puts the exit vector in the site's descriptor, mono splices a per-site copy whose `ret` becomes a jump to the instruction after the call, and hybrid groups the exit-bearing sites reaching one `(routine, composite)` — over the WHOLE reachable set, copies included — and shares under frames iff `k >= 2 && (k-1)*B > Σd`. One closure walk backs both the fold probe and the stamp builder. Symbolic bindings (named entries, glyph labels, open maps, exit vectors) resolve in `linker/interface.rs`, a pre-pass between name resolution and the engine that never mutates an object.
...
**Thin-renderer rule: library code never prints.** Every stage returns a structured report (`CompileReport`, `LinkReport`, `OptReport`, `RunResult`); every byte of terminal output originates in `cli/` (rendered under `-v`), and errors flow as typed values. `bin/pmt.rs` is a shell around `cli::execute`. Fourteen subcommands split across `build.rs` (compile/asm/link), `driver.rs` (build — the manifest-aware compile+link driver, argv mode or `pmt.json` manifest mode), `inspect.rs` (dis/tape-block/ir), `run.rs` (run, incl. live `--trace`), `completions.rs` (completions), `man.rs` (man — the `pmt(1)` page, which `crate::man` renders from the completions registry's subcommand list plus every `--help` text verbatim through `cli::usage_text`, the one table of usage constants; a subcommand missing from that table fails the man module's tests), `lint.rs` (lint — both languages by extension, shared allow namespace), `fmt.rs` (fmt — both languages, stdin via `-` with `--lang`), `lsp.rs` (lsp — the dual-language LSP server on stdio; the only place real stdio is handed to the core server loop), and `dap.rs` (dap — the debug adapter, likewise on stdio). `tmt`'s `cli/` mirrors the split, plus `interface.rs` for its fifteenth subcommand.
...
`pmt completions <shell>` / `tmt completions <shell>` render a script to stdout. Because both CLIs are hand-rolled with no clap, no framework can generate them and they would drift: `completions::registry` is the **single in-crate description of the CLI surface** (every subcommand, each flag's value shape, exclusive groups, positional extension filters) and three renderers read it: `completions::zsh` (an `_arguments -C` script), `completions::bash` (one `_pmt` function for bash 3.2+, no `bash-completion` dependency, re-tokenizing `COMP_LINE` itself so `--emit-ir=after:inline` survives bash's `=`/`:` word breaks), and `completions::fish` (declarative `complete -c` lines). The drift guard cross-checks `--fno-<pass>`/`--emit-ir=after:<pass>` against `optimizer::pass_names()` exactly and probes the real parser with every registry entry.

**The one direction the guard cannot check is a real flag the registry is MISSING** — the hand-rolled parser has no reflection over its match arms. `EXPECTED_TOP_LEVEL`, a hand-maintained mirror of the subcommand set, partly compensates; keep it updated. Separate tests shell out to the real shells: zsh confirms the script parses and loads under `compinit` (interactive candidate correctness there needs a pty and was checked by hand), while bash (`COMP_LINE`/`COMP_POINT` → `_pmt` → `COMPREPLY`) and fish (`complete -C`) are queried headlessly, so their tests assert actual candidates — files by extension, exclusive groups, `=`/`:`-joined values. Each skips with a note when its shell is absent; CI installs fish for that reason (the runner image has bash and zsh). Design doc: `docs/superpowers/specs/2026-07-06-pmt-shell-completion-design.md`.
...
`fmt` is canonical and **whitespace-only** on both source languages: `.pmc` and `.tmc` both print from their green syntax trees (`syntax/`); `.pma`/`.tma` use core's canonical-grid `asm/fmt.rs`. **`.tmc` fmt holds the never-move rule and is unconditionally idempotent** (measured 2026-08-31 over the 61-position audit, `crates/turing-machine/tests/comment_positions.rs`): a comment prints between the same two significant tokens it was written between — headers, statements, list entries, rule action slots — with the grid ruling on rules (a BLOCK comment keeps its rule on the grid occupying its column and widening it for the group; a LINE comment takes the rule off the grid) and THREE recorded residuals that still relocate, all pass-1-stable: a `call` transition's own machinery claims comments past its target (`call /* c */ n::r(`, `) /* c */ then`, `then /* c */ stop`), a `with map`-bearing argument's `with /* c */ map` moves onto the map's `{`, and — recorded 2026-09-21, not in the audit's 61 positions — a comment written between a SIGNATURE PARAMETER and the following `,` crosses the comma (`writes {} /* c */ ,` prints `writes { }, /* c */`), the parameter's own interior, contract clause included, being unaffected. The reference is `docs/tmt/fmt.md` (comments are never moved). **`.pmc` fmt holds the never-move rule too and is unconditionally idempotent** (measured 2026-09-01 over the 39-position audit, `crates/post-machine/tests/comment_positions.rs`, no residuals): header comments print on the header line in their written slot, label-region comments print in the label prefix (the statement takes the own-line-label layout), a pre-`;` comment prints before the `;`, and item-interior/`use`-path comments print inside their item/path — the old stacked-label two-pass settle and the header relocation are both gone. The reference is `docs/pmt/fmt.md` (comments). **The compiled-stdlib byte-identity gate is a NEGATIVE CONTROL for a formatter change, never proof that fmt's output is unchanged** — it runs the compiler over the embedded source (`compile(stdlib::SOURCE, …)`), the formatter is nowhere in that path, and unless the change itself reformats `std.pmc`/`std.tmc` that source is not even touched — so the gate passes with a completely broken printer. Run it anyway, at BOTH opt levels, byte-comparing the object before and after: it proves the change did not leak into the compiler. Take actual byte-identity from the printers' pinned fixtures, each crate's fmt-clean dogfood corpus, and — TM-side — the committed `.fmt` sidecars over the adversarial set plus the audit's three per-position gates (neighbour invariance, token preservation, idempotence). Interior list comments: a comment INSIDE an entry's tokens prints in the entry's text without breaking the list; a BOUNDARY comment keeps its slot (own-line keeps its line, trailing rides the preceding entry, a LINE comment forces multi-line), and the five `paren_list`/`with-map` surfaces still break to multi-line on any boundary comment — a rendering gap, not data loss.
...
- Both JetBrains plugins pin `jvmToolchain(17)`. **Both own their files' PSI through a deliberately minimal parser definition** (word / whitespace / punctuation tokens under one file node, `*ParserDefinitions.kt`) — a language file type WITHOUT one gets a plain-text PSI whose language disagrees with the file's, and LSP4IJ then never sends `didOpen` for it: no hover, no navigation, silently, on every LSP4IJ version (the 0.2.0 regression, found 2026-09-02 by A/B against the July 0.1.x build). Word-grained tokens are also what make the Cmd/Ctrl+hover underline identifier-precise — the platform underlines the PSI element under the caret, so the old "LSP4IJ underlines the whole file" limitation was ours to fix. Each plugin also registers, for its own languages, the per-language LSP4IJ extensions LSP4IJ registers only for plain text/TextMate (semantic-token view provider, Structure view, folding, code blocks, parameter info); a new LSP4IJ per-language extension needs the same mirroring here. Kotlin trap met on the way: inside an `object : PsiFileBase(...)` a bare `fileType` is the object's own synthetic property — the fields are `ownFileType`/`ownLanguage` for that reason.
...
- **The text-expressibility gate**: everything the compiler can put in an object is expressible in hand-written assembly, proven by dis→asm byte-identity. Three declared exceptions, the same three `docs/formats.md` (text-expressibility caveats) lists bullet for bullet: (1) `-g` debug side-tables; (2) exported and imported alphabets — one bullet, two comment forms, both compiler facts with no directive, printed as disassembly comments that reassembling drops; (3) an object with grafts but no interface, a wire-representable combination whose disassembly the assembler refuses. A feature that can only be emitted, never written by hand, breaks it.
...
both editor plugins, their `MIN_TESTED_PMT` floors, and the
`RELEASE_DATE` literal in each crate's `man.rs` (the `.TH` date of the
man page; a test holds it against the CHANGELOG heading for the crate
version, so a bump without it is red) in one commit with the CHANGELOG
entry; merge, tag `vX.Y.Z`, `gh release create` with
...
the job waits up to 30 minutes for the release otherwise. **Plugin
artifacts attach to final release tags only, and only when a plugin's
