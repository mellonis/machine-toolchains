...
tail-sink, tail-call, tail-merge, dce, move-elim, fuse-tape-ops, plus the
program-level inline pass — see `docs/pmt/isa.md` for none of these; they
are compiler internals with no ISA surface, and
`docs/pmt/optimizer.md (passes)` is their reference). Individual passes
can be turned off with `--fno-<pass>` (e.g. `--fno-inline`), repeatable.
