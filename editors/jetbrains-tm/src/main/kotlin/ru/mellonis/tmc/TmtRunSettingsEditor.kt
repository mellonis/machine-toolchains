...
/**
 * The tmt run-configuration editor: subcommand preset combo + args +
 * working directory, plus the build-only row — an editable target combo
 * (fed by `tmt build --list-targets`, run in the working directory on
...
class TmtRunSettingsEditor : SettingsEditor<TmtRunConfiguration>() {
    ...
    private val refreshTargetsButton = JButton("Refresh").apply {
        toolTipText = "Re-read the target list (tmt build --list-targets in the working directory)"
    }
    ...
    private fun refreshTargets() {
        ...
        AppExecutorUtil.getAppExecutorService().submit {
            val result = runCatching { listTmtTargets(cwd) }
