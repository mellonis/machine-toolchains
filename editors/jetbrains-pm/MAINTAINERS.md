# PMC for JetBrains IDEs — maintainer notes

Building the plugin from source and the manual verification checklist
that runs against a built plugin zip. The user-facing page is
`README.md`.
...
`buildPlugin` produces `build/distributions/pmc-0.2.1.zip`. Install it:

1. Settings → Plugins → the ⚙ (gear) icon in the top-right of the
   Plugins page → **Install Plugin from Disk…**
2. Pick `build/distributions/pmc-0.2.1.zip`.
...
- [x] **Go-to-definition**: invoke it (Go to Declaration) on the `L1`
      ...
      `std::` calls do. Confirm too that the Cmd/Ctrl+hover underline
      covers just `L1`, not the whole document.
