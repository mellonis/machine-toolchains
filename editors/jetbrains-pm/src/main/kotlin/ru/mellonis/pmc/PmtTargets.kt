...
data class PmtTarget(val name: String, val runnable: Boolean)

/**
 * Lists the project manifest's declared targets by running
 * `pmt build --list-targets` in [workingDirectory] — the driver does its
 * own nearest-ancestor `pmt.json` discovery from there, so the plugin
 ...
 */
fun listPmtTargets(workingDirectory: String): List<PmtTarget> {
    val commandLine =
        GeneralCommandLine(PmtSettings.instance.state.pmtPath, "build", "--list-targets")
    if (workingDirectory.isNotBlank()) {
        commandLine.withWorkDirectory(workingDirectory)
    }
    val output = CapturingProcessHandler(commandLine).runProcess(LIST_TARGETS_TIMEOUT_MS)
    if (output.isTimeout) {
        throw RuntimeException("pmt build --list-targets timed out")
    }
 ...
 * tests.
 */
internal fun parseTargets(stdout: String): List<PmtTarget> =
    stdout.lineSequence()
        .filter { it.isNotEmpty() }
        .map { line ->
            val pieces = line.split('\t')
            PmtTarget(pieces[0], pieces.getOrNull(1) == "run")
        }
